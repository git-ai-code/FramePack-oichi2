...
transformerモデルのライフサイクル管理を行います。
LoRA設定、FP8最適化、モデルモード（通常/F1）管理を提供します。
"""
...
class TransformerManager:
    ...
    # モデルパス定義（ローカルキャッシュパス使用）
    @property
    def MODEL_PATH_NORMAL(self) -> str:
        return self._get_local_model_path('lllyasviel--FramePackI2V_HY')
    
    @property
    def MODEL_PATH_F1(self) -> str:
        return self._get_local_model_path('lllyasviel--FramePack_F1_I2V_HY_20250503')
    ...
    def _get_local_model_path(self, model_name: str) -> str:
        ...
        model_cache_dir = os.path.join(model_cache_base, 'hub', f'models--{model_name}')
    ...
    def __init__(self, device: Union[str, torch.device], high_vram_mode: bool = False, use_f1_model: bool = False) -> None:
        """
        TransformerManagerインスタンスを初期化します。
        
        Args:
            device: 使用するデバイス
            high_vram_mode: High-VRAMモードの有効/無効
            use_f1_model: F1モデル使用フラグ
        """
    ...
    def _find_model_files(self, model_path):
        ...
        # ローカルパス（フルパス）が渡された場合は直接そのパス内で検索
        ...
        # フォールバック: 元のrepo_id形式での検索
        from common_utils.hf_config import get_model_cache_base_path
        model_root = get_model_cache_base_path()
        ...
        # 見つからない場合は空リストを返す
        print(f"[transformer] モデルファイルが見つかりません: {model_path}")
        return []
        ...
        try:
            ...
            if (not lora_paths) and not self.next_state['fp8_enabled'] and not force_dict_split:
                ...
                if self.next_state['fp8_enabled']:
                    from lora2_utils.fp8_optimization_utils import check_fp8_support
                ...
                try:
                    from lora2_utils.lora_loader import load_and_apply_lora
                ...
                if self.next_state['fp8_enabled']:
                    try:
                        from lora2_utils.fp8_optimization_utils import apply_fp8_monkey_patch
