...
endframe_ichi.pyから外出しされたプリセット関連処理を含みます。
プロンプトプリセットの保存・読み込み・削除機能を提供します。
"""
...
def get_presets_folder_path() -> str:
    """
    プリセットフォルダの絶対パスを取得します。
...
def initialize_presets() -> None:
    """
    初期プリセットファイルがない場合に作成します。
    
    デフォルトプロンプトセットと起動時デフォルトを組み込みます。
    """
    ...
    # デフォルト起動時プロンプト
    default_startup_prompt: str = "A character doing some simple body movements."

    # 既存ファイルがあり、正常に読み込める場合は終了
    if os.path.exists(preset_file):
        try:
            with open(preset_file, 'r', encoding='utf-8') as f:
                presets_data: Dict[str, Any] = json.load(f)

            # 起動時デフォルトがあるか確認
            startup_default_exists: bool = any(preset.get("is_startup_default", False) for preset in presets_data.get("presets", []))
    ...
    while retry_count < max_retries:
        try:
            with open(preset_file, 'r', encoding='utf-8') as f:
                ...
                data: Dict[str, Any] = json.loads(file_contents)
                return data
            ...
            # ファイルをバックアップ
            backup_file: str = f"{preset_file}.bak.{int(datetime.now().timestamp())}"
            try:
                import shutil
...
def get_default_startup_prompt() -> str:
    """
    起動時に表示するデフォルトプロンプトを取得します。
    
    Returns:
        str: 起動時デフォルトプロンプト
    """
    print(translate("起動時デフォルトプロンプト読み込み開始"))
    presets_data: Dict[str, Any] = load_presets()
...
def save_preset(name: Optional[str], prompt_text: str) -> str:
    """
    プリセットを保存します。
    
    Args:
        name: プリセット名（Noneの場合は起動時デフォルトとして保存）
    ...
    if not name:
        ...
        # 既存の起動時デフォルトを探す
        startup_default_exists: bool = False
        for preset in presets_data["presets"]:
            if preset.get("is_startup_default", False):
                # 既存の起動時デフォルトを更新
...
def delete_preset(preset_name: str) -> str:
    """
    プリセットを削除します。
